              ...
              >
                ...
                <div className={`hidden md:flex items-center ${index % 2 === 0 ? 'flex-row' : 'flex-row-reverse'}`}>
                  <div className={`w-1/2 ${index % 2 === 0 ? 'pr-8 lg:pr-12 text-right' : 'pl-8 lg:pl-12 text-left'}`}>
                    <h3 className="text-lg lg:text-xl font-bold text-blue-200">{event.title}</h3>
                    <p className="text-gray-300 mt-2 text-sm lg:text-base">{event.result}</p>
                    <a href={event.link} className="inline-block mt-2 text-secondary hover:text-accent transition-colors duration-300 text-sm">
                      See More
                    </a>
                  </div>
                ...
                <div className="md:hidden flex items-start space-x-4">
                  <div className="flex flex-col items-center flex-shrink-0">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-secondary to-accent flex items-center justify-center z-20 shadow-lg shadow-accent/30">
                      <span className="text-xs font-bold">{event.year}</span>
                    </div>
                    
                    {/* Glowing connection effect */}
                    <motion.div 
                      className="absolute w-6 h-6 rounded-full bg-accent/30 blur-md z-10"
                      animate={{
                        scale: [1, 1.2, 1],
                        opacity: [0.3, 0.6, 0.3]
                      }}
                  ...
                  <div className="flex-1 text-left">
                    <h3 className="text-lg font-bold text-blue-200">{event.title}</h3>
                    <p className="text-gray-300 mt-1 text-sm">{event.result}</p>
                    <a href={event.link} className="inline-block mt-2 text-secondary hover:text-accent transition-colors duration-300 text-sm">
                      See More
                    </a>
                  </div>
